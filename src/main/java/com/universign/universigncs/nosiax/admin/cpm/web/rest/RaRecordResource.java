...
public class RaRecordResource {
  ...
  public RaRecordResource(RaRecordService raRecordService, RaRecordRepository raRecordRepository) {
  ...
  public ResponseEntity<RaRecord> createRaRecord(@Valid @RequestBody RaRecord raRecord) throws URISyntaxException {
    ...
    return ResponseEntity
      .created(new URI("/api/ra-records/" + result.getId()))
      .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, result.getId().toString()))
      .body(result);
  }

  /**
   * {@code PUT  /ra-records/:id} : Updates an existing raRecord.
   *
   * @param id the id of the raRecord to save.
   ...
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/ra-records/{id}")
  public ResponseEntity<RaRecord> updateRaRecord(
    @PathVariable(value = "id", required = false) final Long id,
    @Valid @RequestBody RaRecord raRecord
  ) throws URISyntaxException {
    log.debug("REST request to update RaRecord : {}, {}", id, raRecord);
    if (raRecord.getId() == null) {
      throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
    }
    if (!Objects.equals(id, raRecord.getId())) {
      throw new BadRequestAlertException("Invalid ID", ENTITY_NAME, "idinvalid");
    }

    if (!raRecordRepository.existsById(id)) {
  ...
  /**
   * {@code PATCH  /ra-records/:id} : Partial updates given fields of an existing raRecord, field will ignore if it is null
   *
   * @param id the id of the raRecord to save.
   * @param raRecord the raRecord to update.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated raRecord,
   * or with status {@code 400 (Bad Request)} if the raRecord is not valid,
   * or with status {@code 404 (Not Found)} if the raRecord is not found,
   * or with status {@code 500 (Internal Server Error)} if the raRecord couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(value = "/ra-records/{id}", consumes = { "application/json", "application/merge-patch+json" })
  public ResponseEntity<RaRecord> partialUpdateRaRecord(
    @PathVariable(value = "id", required = false) final Long id,
    @NotNull @RequestBody RaRecord raRecord
  ) throws URISyntaxException {
    log.debug("REST request to partial update RaRecord partially : {}, {}", id, raRecord);
    if (raRecord.getId() == null) {
      throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
    }
    if (!Objects.equals(id, raRecord.getId())) {
    ...
    Optional<RaRecord> result = raRecordService.partialUpdate(raRecord);

    return ResponseUtil.wrapOrNotFound(
      result,
      HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, raRecord.getId().toString())
    );
  }
  ...
  public ResponseEntity<List<RaRecord>> getAllRaRecords(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
