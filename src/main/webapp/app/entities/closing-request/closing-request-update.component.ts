...
export class ClosingRequestUpdateComponent implements OnInit {
  ...
  editForm = this.fb.group({
    ...
    terminate: [],
    terminateDate: [],
    idTransactionClosing: [],
    urlTransactionClosing: [],
    agency: []
  });
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ closingRequest }) => {
      if (!closingRequest.id) {
        ...
        closingRequest.idxAgency = today;
      ...
      this.agencyService
        ...
        .subscribe((resBody: IAgency[]) => {
          if (!closingRequest.agency || !closingRequest.agency.id) {
            this.agencies = resBody;
          } else {
            this.agencyService
              .find(closingRequest.agency.id)
              .pipe(
                map((subRes: HttpResponse<IAgency>) => {
                  return subRes.body ? [subRes.body].concat(resBody) : resBody;
                })
              )
              .subscribe((concatRes: IAgency[]) => (this.agencies = concatRes));
          }
        });
    });
  }
  ...
  updateForm(closingRequest: IClosingRequest): void {
    this.editForm.patchValue({
      ...
      idxAgency: closingRequest.idxAgency ? closingRequest.idxAgency.format(DATE_TIME_FORMAT) : null,
      ...
      idTransactionClosing: closingRequest.idTransactionClosing,
      urlTransactionClosing: closingRequest.urlTransactionClosing,
      agency: closingRequest.agency
    });
  }
  ...
  private createFromForm(): IClosingRequest {
    return {
      ...
      idxAgency: this.editForm.get(['idxAgency'])!.value ? moment(this.editForm.get(['idxAgency'])!.value, DATE_TIME_FORMAT) : undefined,
      ...
      terminateDate: this.editForm.get(['terminateDate'])!.value
        ...
        : undefined,
      idTransactionClosing: this.editForm.get(['idTransactionClosing'])!.value,
      urlTransactionClosing: this.editForm.get(['urlTransactionClosing'])!.value,
